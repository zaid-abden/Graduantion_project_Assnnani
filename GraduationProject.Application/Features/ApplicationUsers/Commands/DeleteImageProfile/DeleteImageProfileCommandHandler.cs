...
namespace GraduationProject.Application.Features.ApplicationUsers.Commands.DeleteImageProfile
{
    public class DeleteImageProfileCommandHandler : IRequestHandler<DeleteImageProfileCommand, Result<string>>
    {
        ...
        public DeleteImageProfileCommandHandler(UserManager<User> userManager, IFileServices fileService, ICurrentUserService currentUserService)
        {
        ...
        public async Task<Result<string>> Handle(DeleteImageProfileCommand request, CancellationToken cancellationToken)
        {
            ...
            var removeImage = fileService.Remove(user.ImageUrl);
